...
export async function deployWalletNMT() {
    ...
    const WalletNMT = await ethers.getContractFactory("WalletNMT");
    const walletNMT = await WalletNMT.deploy(owner.address);
...
export async function deployWalletAsset() {
  // Contracts are deployed using the first signer/account by default
  ...
  // WalletNMT
  const WalletNMT = await ethers.getContractFactory("WalletNMT");
  const walletNMT = await WalletNMT.deploy(creator.address);
  // HolderSmartPolicy
  ...
  //WalletMutableAsset mint
  const mintTx = await walletNMT.mint(
    buyer.address,
    creatorSmartPolicy.address,
    denyAllSmartPolicy.address
  );
  const mintResponse = await mintTx.wait();
  const walletTokenId = mintResponse.events[0].args["tokenId"];
  const walletAddress = await walletNMT.getWalletAddress(walletTokenId);
  //WalletMutableAsset
  const WalletMutableAsset = await ethers.getContractFactory(
    "WalletMutableAsset"
  );
  const walletMutableAsset = WalletMutableAsset.attach(walletAddress);
