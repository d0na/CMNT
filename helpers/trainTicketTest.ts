...
export async function deployTrainTicketNMT() {
    ...
    const TrainTicketNMT = await ethers.getContractFactory("TrainTicketNMT");
    const trainTicketNMT = await TrainTicketNMT.deploy(owner.address);
...
export async function deployTrainTicketAsset() {
  // Contracts are deployed using the first signer/account by default
  ...
  // TrainTicketNMT
  const TrainTicketNMT = await ethers.getContractFactory("TrainTicketNMT");
  const trainTicketNMT = await TrainTicketNMT.deploy(creator.address);
  // HolderSmartPolicy
  ...
  //TrainTicketMutableAsset mint
  const mintTx = await trainTicketNMT.mint(
    buyer.address,
    creatorSmartPolicy.address,
    denyAllSmartPolicy.address
  );
  const mintResponse = await mintTx.wait();
  const trainTicketTokenId = mintResponse.events[0].args["tokenId"];
  const trainTicketAddress = await trainTicketNMT.getTrainTicketAddress(trainTicketTokenId);
  //TrainTicketMutableAsset
  const TrainTicketMutableAsset = await ethers.getContractFactory(
    "TrainTicketMutableAsset"
  );
  const trainTicketMutableAsset = TrainTicketMutableAsset.attach(trainTicketAddress);
